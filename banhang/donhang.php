...
`iduser`, `diachi`, `sodienthoai`, `email`, `soluong`, `tongtien`) VALUES ("'.$iduser.'","'.$diachi.'","'.$sdt.'","'.$email.'","'.$soluong.'","'.$tongtien.'")';
$data = mysqli_query($conn, $query);
if($data == true){
	$query = 'SELECT id iddonhang FROM `donhang` WHERE `iduser` = '.$iduser.' ORDER BY id DESC LIMIT 1';
	$data = mysqli_query($conn, $query);
